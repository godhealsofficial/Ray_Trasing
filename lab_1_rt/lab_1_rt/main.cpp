...
void create_scene()
{
    ...
    auto sphere1 = std::make_shared<Sphere>(float3(+6.5f, +5.0f, +2.65f), 4, new IdealMirror(float3(1.0f, 1.0f, 1.0f)));
    myScene.geoObjects.push_back(sphere1);

    auto sphere2 = std::make_shared<Sphere>(float3(-5.5f, +3.0f, -13.5f), 5, new Diffuse(float3(0.8f, 0.3f, 0.7f)));
